...
pub async fn get_votes_delete(sql: &Sql, delete_card_id: &Id) -> Result<i32, sqlx::Error> {
    let (count, ): (i32, ) = sqlx::query_as(
        "SELECT 
         CAST(COALESCE(SUM(dcvtype), 0) AS SIGNED)
         FROM deletecardvotes
         WHERE dcid = ?;")
        .bind(delete_card_id)
        .fetch_one(sql.pool())
        .await?;
...
pub async fn get_vote_delete(sql: &Sql, delete_card_id: &Id, user_id: &Id) -> Result<CardVote, sqlx::Error> {
    let stmt: Result<(i32, ), sqlx::Error> = sqlx::query_as(
        "SELECT dcvtype
         FROM deletecardvotes
         WHERE dcid = ?
