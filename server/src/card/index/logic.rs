...
pub async fn card_index_route(collector_id: Id, sql: &State<Sql>, search: Option<String>, page: Option<u32>, state: Option<i32>, sort_type: Option<i32>, config: &State<Config>) -> ApiResponseErr<CardIndexResponse> {
    ...
    let cards = rjtry!(card::sql::get_cards(&sql, &collector_id, search.clone(), &sort_type, config.card_type_page_amount, page * config.card_type_page_amount, card_state.clone()).await);
