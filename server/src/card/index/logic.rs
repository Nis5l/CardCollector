...
pub async fn card_index_route(collector_id: Id, sql: &State<Sql>, search: Option<String>, page: Option<u32>, state: Option<CardState>, sort_type: Option<CardSortType>, config: &State<Config>) -> ApiResponseErr<CardIndexResponse> {
    let page = page.unwrap_or(0);
    let search = search.unwrap_or(String::from(""));
    let sort_type = sort_type.unwrap_or(CardSortType::default());
